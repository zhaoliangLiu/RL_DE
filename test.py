...
def test_model(dim, function_id, model_path="ea_ppo_model"):
    # 配置不同维度的测试参数
    dim_config = {
    ...
    # 确保输出目录存在
    os.makedirs(f'graph/勘探开发/{dim}D', exist_ok=True)
    os.makedirs(f'data/勘探开发/{dim}D', exist_ok=True)
    ...
    # 保存结果
    plt.tight_layout()
    plt.savefig(f'graph/勘探开发/{dim}D/f{function_id}.png', bbox_inches='tight', dpi=300)
    ...
    # 确保目录存在
    if not os.path.exists(f'data/勘探开发/{dim}D'):
        os.makedirs(f'data/勘探开发/{dim}D', exist_ok=True)

    # 保存为CSV文件
    csv_path = f'data/勘探开发/{dim}D/F{function_id}.csv'

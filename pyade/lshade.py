...
def apply(population_size: int, individual_size: int, bounds: np.ndarray,
    ...
    Second column represent the minimum value for the row feature.
    ...
    :return: A pair with the best solution found, its fitness and the history of best fitness values.
    ...
    fitness_history.append(fitness[best_index])  # Record initial best fitness
    ...
    while num_evals < max_evals:
        ...
        if len(indexes) > 0:
            ...
            m_cr[k] = np.sum(weights * cr[indexes] ** 2) / (np.sum(weights * cr[indexes]) + 1e-10)
            if np.isnan(m_cr[k]):
                m_cr[k] = 1
            m_f[k] = np.sum(weights * f[indexes] ** 2) / (np.sum(weights * f[indexes]) + 1e-10)
            k += 1
            if k == memory_size:
                k = 0
